...
class SubscriptionSourceService {
  ...
  async get(id: string): Promise<SubscriptionSource | null> {
    const results = await db.select().from(subscriptionSources).where(eq(subscriptionSources.id, id)).limit(1);
  ...
  async update(id: string, data: Partial<Omit<SubscriptionSource, "id" | "createdAt" | "updatedAt">>): Promise<SubscriptionSource> {
    ...
    const results = await db.update(subscriptionSources).set(updateData).where(eq(subscriptionSources.id, id)).returning();
