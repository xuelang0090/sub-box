...
class SubscriptionSourceItemService {
  ...
  async get(id: string): Promise<SubscriptionSourceItem | null> {
    const results = await db.select().from(subscriptionSourceItems).where(eq(subscriptionSourceItems.id, id)).limit(1);
  ...
  async update(id: string, data: Partial<Omit<SubscriptionSourceItem, "id" | "createdAt" | "updatedAt">>): Promise<SubscriptionSourceItem> {
    ...
    const results = await db.update(subscriptionSourceItems).set(updateData).where(eq(subscriptionSourceItems.id, id)).returning();
